...
public class MixSpecies {
    ...
    public MixSpecies(int lmin, int lmax, double sigma, double mu) {
        ...
        N = 0;
        for (int l = 0; l < nc; l++) {
            N += (int) Math.pow(m, len[l]);
        }
        seq = new int[N][];
        int s = -1;
        for (int l = 0; l < nc; l++) {
            int max = (int) Math.pow(m, len[l]);
            for (int i = 0; i < max; i++) {
                s++;
                seq[s] = new int[len[l]];
            }
    ...
    /**
     * combines constraints on vector of species abundances
     */
     ...
     * digest constraints
     */
    public LinEqCons getRhoLHomFeasLEC(BBSet bbs, String[] csFile, String[] fragFile) {
        List<LinEqCons> v = new ArrayList<>();
        v.add(getNormLEC());
        for (int i = 0; i < csFile.length; i++) {
            CSpec cs = new CSpec(csFile[i], bbs);
     ...
     */
    public LinEqCons getRhoLHomLEC(BBSet bbs) {
    ...
    /**
     * linear equality constraint for fragment length distribution
     */
    ...
    /**
     * contribution of species s to expected number of fragments of length ll
     */
    double getFragContrib(int s, int ll, CSpec cs) {
        int n_seq = seq[s].length;
        double res = 0.0;
        if (ll > n_seq) {
            return res;
        }
        for (int i = 1; i < n_seq - ll; i++) {
            double t = cs.c[seq[s][i]];
            for (int j = 1; j <= ll - 1; j++) {
                t *= (1.0 - cs.c[seq[s][i + j]]);
            }
            t *= cs.c[seq[s][i + ll]];
            res += t;
        }
        double t = cs.c[seq[s][n_seq - ll]];
        for (int j = n_seq - ll + 1; j < n_seq; j++) {
            t *= (1.0 - cs.c[seq[s][j]]);
        }
        res += t;
        return res;
    ...
    /**
     * linear equality constraint for overall disaccharide composition
     */
    public LinEqCons getRhoLEC(BBSet bbs) {
